/*
  RString.h - modified by Thomas Euler using ...
...
public:
  ...
  RString(char *buf, size_t size, int pos) : _buf(buf), _size(size), _pos(pos)
  { begin(); }

  // templated constructors allow inline renderings of this type: RString(buf, size, myfloat[, modifier]);
  template<class T> RString(char *buf, size_t size, T arg) : _buf(buf), _size(size) 
  { begin(); print(arg); }
  
  template<class T> RString(char *buf, size_t size, T arg, int modifier) : _buf(buf), _size(size) 
  ...
  template<class T> inline RString &operator =(T arg) 
